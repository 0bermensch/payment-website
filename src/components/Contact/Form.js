...
const Form = () => {
    ...
    inputElements.forEach((input) =>
      input.addEventListener("focusin", () => {
        if (input.children[0].type !== "email") {
          ...
          input.children[0].type === "email" &&
          !validateEmail(input.children[0].value) &&
  ...
  return (
    ...
    >
      ...
      <div className="contact__form__control">
        <textarea aria-label="Enter message here" placeholder="Message" />
        <small className="error__msg" />
      </div>
      <div className="contact__form__control">
        <span
          className={`checkbox ${emailList ? "checked" : ""}`}
          onClick={() => {
        ...
        <small
          className={`email__list ${emailList ? "checked" : ""}`}
          onClick={() => {
            setEmailList(!emailList);
          }}
        >
      ...
      <button
        className={`btn btn--transparent-blue ${validForm ? "valid" : ""}`}
        type="submit"
      >
        Submit
      </button>
